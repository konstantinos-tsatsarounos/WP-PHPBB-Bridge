...
function get_forum_avatar_by_id( $user_id, $size ){
    ...
    if($cn && $user->data->user_email != '' )
    {
        if(@mysql_select_db($dbname))
        {
            $qr = "SELECT u.`user_avatar`, u.`user_avatar_type` FROM `" . $table_prefix  . "users` u WHERE u.`user_email` = '" . $user->data->user_email . "'";
            $rs = @mysql_query($qr, $cn);
        }
        
        while($i = mysql_fetch_assoc($rs))
        {
        	if( wpbb_get_avatar($i['user_avatar'], $i['user_avatar_type']) != '' ) {
	            $avatar = '<img 
	                alt="" 
	                src="' . wpbb_get_avatar($i['user_avatar'], $i['user_avatar_type']) . '" 
	                class="avatar avatar-' . $size . ' photo" 
